...
impl Widget for DragValue<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let drag_to_edit = ui.style().interaction.drag_value_dragging;
        ...
        let mut response = if is_kb_editing {
            ...
            let button = Button::new(atoms)
                ...
                .sense(if drag_to_edit { Sense::click_and_drag() } else { Sense::click() })
                .gap(0.0)
                .min_size(ui.spacing().interact_size); // TODO(emilk): find some more generic solution to `min_size`

            let cursor_icon = if !drag_to_edit {
                CursorIcon::Text
            ...
            if response.clicked() {
                ...
                select_all_text(ui, id, response.id, &value_text);
            } else if drag_to_edit && response.dragged() {
