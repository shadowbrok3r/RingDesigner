...
use ringdesign_core::castability::section_at;
use ringdesign_core::field::{FieldContext, Layer, LayerEntry, LayerStack, SeatStyle, Uv};
...
mod tests {
    use super::*;
    use ringdesign_core::field::{SeatPadLayer, SeatRunLayer};
    use ringdesign_core::LayerEntry;
    ...
    fn stones_land_on_their_seats() {
        ...
        if let Ok(dir) = std::env::var("RD_GEM_SHEET") {
            ...
            for (i, v) in verts.chunks(12).enumerate() {
                mesh.vertices.push(ringdesign_core::Vec3(v[0], v[1], v[2]));
                mesh.normals.push(ringdesign_core::Vec3(v[3], v[4], v[5]));
                if i % 3 == 2 {
                    let k = base + i as u32;
