//! A factory stock made into a two-part sand master: its upper half mirrored about the parting plane, refined, and drafted toward the pull.
use super::Source;
use crate::field::smoothstep;
use anyhow::{Result, ensure};
use std::{collections::HashMap, sync::Arc};

/// Longest edge the refined master keeps, mm.
const MASTER_EDGE_MM: f64 = 0.55;
/// Most faces a master may carry.
const MASTER_MAX_FACES: usize = 400_000;

/// Split every edge longer than [`MASTER_EDGE_MM`] at its midpoint until none is left.
fn refine(s: &mut Source) -> Result<()> {
    for _ in 0..10 {
        ...
        for f in &s.faces {
            for (a, b) in [(f[0], f[1]), (f[1], f[2]), (f[2], f[0])] {
                ...
                if p.iter().zip(q).map(|(a, b)| (a - b).powi(2)).sum::<f64>() > MASTER_EDGE_MM.powi(2) {
                    mids.insert(key, s.vertices.len() as u32);
                    s.vertices.push(std::array::from_fn(|i| (p[i] + q[i]) * 0.5));
                }
            }
        }
        if mids.is_empty() {
        ...
        for [a, b, c] in s.faces.drain(..) {
            ...
            match (get(a, b), get(b, c), get(c, a)) {
                ...
                (Some(d), Some(e), Some(f)) => faces.extend([[a, d, f], [d, b, e], [f, e, c], [d, e, f]]),
            }
        }
        s.faces = faces;
        ensure!(s.faces.len() <= MASTER_MAX_FACES, "Drafted master exceeds source budget");
    }
    Ok(())
}

/// The sand master of a stock: the half above its mid-plane clipped, mirrored below it, refined and drafted toward the pull, deterministic.
pub fn sand_master(source: Arc<Source>) -> Result<Arc<Source>> {
    let mut s: Source = serde_json::from_value(serde_json::to_value(&*source)?)?;
    let bore = s.calibration.bore_radius_mm;
    let min_z = s.vertices.iter().map(|p| p[2]).fold(f64::INFINITY, f64::min);
    let max_z = s.vertices.iter().map(|p| p[2]).fold(f64::NEG_INFINITY, f64::max);
    let z0 = (min_z + max_z) * 0.5;
    let old = s.vertices.clone();
    let old_faces = s.faces.clone();
    s.vertices.clear();
    s.faces.clear();
    // Vertices welded at a micron, numbered in the order the faces first reach them.
    ...
    refine(&mut s)?;
    // Half the section's axial reach per whole degree round the ring, widened 8° either way.
    ...
    for p in &mut s.vertices {
        ...
        let head = smoothstep(bore + s.calibration.head_height_mm * 0.35, bore + s.calibration.head_height_mm * 0.8, p[1]);
        let drafted = r + (0.045 + 0.16 * head) * (h - (p[2] * p[2] + 0.36).sqrt()).max(0.);
        let inner = bore + 0.012 * p[2].abs();
        let rr = if r < bore + 0.65 { inner * (1. - skin) + drafted * skin } else { drafted };
        p[0] *= rr / r;
        p[1] *= rr / r;
    }
    ...
    s.calibration.palm_thickness_mm = s.vertices.iter().filter(|p| p[1] < -bore && p[0].abs() < 0.12).map(|p| p[0].hypot(p[1]) - bore).fold(0_f64, f64::max);
