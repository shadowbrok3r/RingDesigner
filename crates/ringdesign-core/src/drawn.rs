...
impl Stroke {
    pub fn new(radius: f32, soft: f32, erase: bool) -> Self {
        Self { points: Vec::new(), radius, soft, erase }
    }
...
impl DrawnAlpha {
    ...
    fn draw_stroke(&self, buf: &mut [f32], w: usize, h: usize, stroke: &Stroke) {
        ...
        match stroke.points.len() {
            ...
            1 => {
                ...
                self.stamp(buf, w, h, p[0], p[1], r, stroke.soft, p[2], stroke.erase);
            }
            _ => {
    ...
    /// One feathered disc. Paint takes the max toward `value`, erase takes the min toward 0, so
    ...
    fn stamp(
        ...
        let x0 = ((cx - rx) * wf).floor() as i64;
        let x1 = ((cx + rx) * wf).ceil() as i64;
        let y0 = ((cy - ry) * hf).floor() as i64;
        let y1 = ((cy + ry) * hf).ceil() as i64;
